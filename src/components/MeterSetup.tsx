              ...
              {profile?.meter_number && provider === 'KPLC' && (
                <div className="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                  ...
                  <p className="text-xs text-yellow-700 mb-3">
                    Test if the USSD webhook is working with your meter: {profile.meter_number}
                  </p>
                  <Button
                    onClick={async () => {
                      if (!profile.phone_number) {
                        toast({
                          ...
                          description: "Please add your phone number to test USSD integration.",
                          variant: "destructive"
                        });
                        return;
                      }
                      ...
                      try {
                        ...
                        toast({
                          ...
                          description: "Check your phone for USSD response. Data will appear in dashboard if successful.",
                          variant: "default"
                        });
                      } catch (error) {
                        console.error('USSD test error:', error);
                        toast({
                          title: "USSD Test Failed",
                          description: error.message || "Failed to send USSD request",
                          variant: "destructive"
                        });
                      } finally {
                        setIsLoading(false);
                    ...
                    className="w-full"
                  >
                    {isLoading ? 'Testing...' : 'Test USSD Balance Check'}
                  </Button>
                </div>
              )}
                  ...
                  <Button
                    onClick={async () => {
                      try {
                        ...
                        console.error('Smart meter test error:', error);
                        toast({
                          title: "Smart Meter Test Failed",
                          description: error.message || "Failed to send test data to smart meter",
                          variant: "destructive"
                        });
                      } finally {
                        setIsLoading(false);
