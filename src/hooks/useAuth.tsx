...
const SESSION_VALIDITY_CHECK_INTERVAL = 60 * 60 * 1000; // Check every 60 minutes (increased from 45)
const SESSION_PERSISTENCE_KEY = 'supabase.session';
// Add a minimum interval between session checks to prevent rate limiting
const MIN_SESSION_CHECK_INTERVAL = 10 * 60 * 1000; // Minimum 10 minutes between checks (increased from 5)
...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const setOnAuthSuccess = useCallback((callback: () => void) => {
    console.log('Setting auth success callback');
  ...
  const checkSessionValidity = useCallback(async () => {
    ...
    const now = Date.now();
    if (now - lastSessionCheck.current < MIN_SESSION_CHECK_INTERVAL) { // 10 minutes minimum between checks
    ...
    try {
      ...
      // If session expires in less than 30 minutes, show warning but don't auto-refresh
    ...
    const initializeAuth = async () => {
      try {
        ...
        if (error) {
          ...
          // Call success callback
          if (onAuthSuccessCallback.current) {
            console.log('Calling auth success callback for fresh session');
            onAuthSuccessCallback.current();
          }
        } else {
      ...
      switch (event) {
        case 'SIGNED_IN':
          if (newSession) {
            ...
            // Execute auth success callback
            if (onAuthSuccessCallback.current) {
              console.log('Executing auth success callback after SIGNED_IN event');
              try {
                onAuthSuccessCallback.current();
                console.log('Auth success callback executed successfully');
              } catch (error) {
                console.error('Error executing auth success callback:', error);
              }
            } else {
              console.log('No auth success callback registered');
            }
          }
          break;
